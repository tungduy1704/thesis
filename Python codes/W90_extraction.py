...
def extract_pcv(filename="wannier90_hr.dat", nk=100, out_csv="p_cv_map.csv", out_eigen_csv="eigenvalues_map.csv"):
    ...
    ky_vals = 0#np.linspace(-kmax, kmax, nk)
    ...
    for kx, ky in tqdm(zip(kx_list, ky_list), total=len(kx_list), desc="Đang quét k-lưới"):
        ...
        #p_bloch_x = U @ px @ U.conj().T
        #p_bloch_y = U @ py @ U.conj().T
...
#extract_pcv(filename="mos2_hr.dat", nk=199, out_csv="p_cv_map.csv", out_eigen_csv="eigenvalues_map.csv")
#extract_pcv(filename="mos2_hr15.dat", nk=49, out_csv="p_cv_map15.csv", out_eigen_csv="eigenvalues_map15.csv")
#extract_pcv(filename="mos2_hr55.dat", nk=49, out_csv="p_cv_map55.csv", out_eigen_csv="eigenvalues_map55.csv")
df1 = pd.read_csv("eigenvalues_map294.csv")
df1.to_csv("eigenvalues_map294.txt", sep=" ", index=False)
df2 = pd.read_csv("eigenvalues_map.csv")
df2.to_csv("eigenvalues_map.txt", sep=" ", index=False)
df3 = pd.read_csv("p_cv_map294.csv")
df3.to_csv("p_cv_map294.txt", sep=" ", index=False)
df4 = pd.read_csv("p_cv_map.csv")
df4.to_csv("p_cv_map.txt", sep=" ", index=False)
df5 = pd.read_csv("p_cv_map55.csv")
df5.to_csv("p_cv_map55.txt", sep=" ", index=False)